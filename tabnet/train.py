...
class TabNet(object):
        ...
        for c_epoch in range(epochs):
            ...
            for batch_idx, (x_batch_cont, x_batch_cat, y_batch) in enumerate(
                ...
                if reconstruction_loss:
                    self.logger.log_metric("{}/reconstruction loss".format(print_stub_name), reconstruction_loss)
                if criterion_loss:
                    self.logger.log_metric("{}/criterion loss".format(print_stub_name), criterion_loss)
                    self.logger.log_metric("{}/sparsity loss".format(print_stub_name), sparsity_loss)
                ...
                self.logger.log_metric("{}/total loss".format(print_stub_name), loss)
            ...
            if val_generator is not None:
                if self_supervised:
                    ...
                    self.logger.log_metric("{}/{}".format(print_stub_name, metric_name), metric_value)
                    self.logger.log_metric("{}/validation criterion loss".format(print_stub_name), criterion_val_loss)
                    print(
    ...
    def predict_proba(self, X, batch_size=1024):
        if not self.model_params["discrete_outputs"]:
            ...
            ret_data.columns = map_ordinals_to_categoricals(
